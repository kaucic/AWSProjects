...
def getAllMysfits():
    ...
    #logging.info(f'Mysfits returned {response["Items"]}')
...
def queryMysfitItems(filter, value):
    logging.info(f"Here filter = {filter} value = {value}")
...
def queryMysfits(queryParam):
    logging.info(f"queryParam {json.dumps(queryParam)}")
