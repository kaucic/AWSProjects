...
def lambda_handler(event, context):
    ...
    if action == 'init_game':
        ...
        init_ret = init_game(gameID)
        print(f"getting gameState: {init_ret}")
        game_body = init_ret
        
        viewStateList.clear()
        viewStateList.append(init_ret.copy())
        game_body['viewStateList'] = viewStateList.copy()
        game_body['viewStateIndex'] = 0
        ...
        if roll_response['valid']:
            ...
            newRollState = roll_response
            viewStateList.append(roll_response.copy())
            newRollState['viewStateList'] = viewStateList.copy()
        ...
        newBankState = bank_response
        viewStateList.append(bank_response.copy())
        newBankState['viewStateList'] = viewStateList.copy()
        newBankState['viewStateIndex'] = gameState['viewStateIndex'] + 1
...
def init_game(game_id):
    ...
    global NDICE, NPlayers, playerNames, player, diceVals, previouslyKeptDice, turnScore, totals
    #gameID += 1
    player = 1
    previouslyKeptDice = [False for x in range(NDICE)]
    diceVals = [1 for x in range(NDICE)]
