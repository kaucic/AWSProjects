...
function initGame(){
 ...
 function rollDice() {
    ...
    for (i=0; i < NDICE; i++) {
      ...
      var lambdaBody = JSON.stringify({"action":"roll_dice", "playerID":playerID, "game_id": game_id, "keptDice": keep});
      lambdaCall(lambdaBody)
	    .then(updateRoll);
  }
  ...
  async function getGameState(){
    // create a JSON object with parameters for API call and store in a variable
    var lambdaBody = JSON.stringify({"action":"get_game_state", "game_id": game_id});
  ...
  function bankScore(){
    // create a JSON object with parameters for API call and store in a variable
    var lambdaBody = JSON.stringify({"action":"bank", "playerID":playerID, "game_id": game_id});
