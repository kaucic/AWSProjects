...
def init():
    ...
    global NPlayers, player, keptDie, wins, total
    player = 1
    keptDie = [0 for x in range(3)]
    wins = [0 for x in range(NPlayers+1)]
    total = [0 for x in range(NPlayers+1)]
...
def roll_dice():
    global NPlayers, player, keptDie, total
    keep = [-1 for x in range(3)]

    # For GET invocations
    if request.method == 'GET':
        logging.info(f"roll_dice GET called")
        # Parse the input parameters
        # Example ?keep1=0&keep2=0&keep3=1
        keep[0] = request.args.get('keep1')
        keep[1] = request.args.get('keep2')
        keep[2] = request.args.get('keep3')
        logging.info(f"Dice to keep: keep1={keep[0]} keep2={keep[1]} keep3={keep[2]}")
    elif request.method == 'POST':
        logging.info(f"roll_dice POST called")
        data = request.get_json()
        logging.info(f"roll_dice received json {data}")
        keep[0] = data['keep1']
        keep[1] = data['keep2']
        keep[2] = data['keep3']
    else:
    ...
    body = { 'player' : player, 'die1' : die[0], 'die2' : die[1], 'die3' : die[2], 'total' : total[player] }
...
def bank_score():
    global NPlayers, player, wins, total
